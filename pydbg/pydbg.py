...
class pydbg:
    ...
    def __init__ (self, ff=True, cs=False):
        ...
        # self._log = lambda msg: utils.dbgPrint(msg, Fore.GREEN, verbose=self.verbose)
        ...
        utils.dbgPrint("[*] System page size is %d" % self.page_size, Fore.GREEN, verbose=self.verbose)

    def setVerbose(self, verbose):
        # utils.dbgPrint("[*] Debug engine set to verbose", Fore.GREEN)
    ...
    def attach (self, pid):
        ...
        # utils.dbgPrint("[*] Attaching to pid %d" % pid)
    ...
    def bp_del (self, address):
        ...
        utils.dbgPrint("[*] Removing breakpoint at 0x%08x" % address, Fore.GREEN, verbose=self.verbose)
    ...
    def bp_del_mem (self, address):
        ...
        utils.dbgPrint("[*] Deleting memory breakpoint at 0x%08x" % address, Fore.GREEN, verbose=self.verbose)
    ...
    def bp_set (self, address, description="", restore=True, handler=None):
        ...
        utils.dbgPrint("[*] Setting breakpoint at 0x%08x\n" % address, Fore.GREEN, verbose=self.verbose)
    ...
    def bp_set_hw (self, address, length, condition, description="", restore=True, handler=None):
        ...
        *Warning: Setting hardware breakpoints during the first system breakpoint will be removed upon process
        ...
        utils.dbgPrint("[*] Setting hardware breakpoint at %08x, %d, %s" % (address, length, condition), Fore.GREEN, verbose=self.verbose)
    ...
    def bp_set_mem (self, address, size, description="", handler=None):
        ...
        utils.dbgPrint("[*] Memory breakpoint buffer range is %08x - %08x" % (address, address + size), Fore.GREEN, verbose=self.verbose)

        # ensure the target address doesn't already sit in a memory breakpoint range:
        if self.bp_is_ours_mem(address):
            utils.dbgPrint("[*] A memory breakpoint spanning %08x already exists" % address, Fore.GREEN, verbose=self.verbose)
            return self.ret_self()
        ...
        utils.dbgPrint("[*] Buffer starting at %08x sits on page starting at %08x" % (address, mbi.BaseAddress), Fore.GREEN, verbose=self.verbose)
        ...
        while current_page <= address + size:
            utils.dbgPrint("[*] Changing page permissions on %08x" % current_page, Fore.GREEN, verbose=self.verbose)
    ...
    def debug_event_iteration (self):
        ...
        if kernel32.WaitForDebugEvent(byref(dbg), 100):
            ...
            # an exception was caught.
                ...
                utils.dbgPrint("[*] Got exception: %08x" % ec, Fore.GREEN, verbose=self.verbose)
                ...
                # unhandled exception.
                    ...
                    utils.dbgPrint("[*] TID:%04x caused an unhandled exception (%08x) at %08x" % (self.dbg.dwThreadId, ec, self.exception_address), Fore.GREEN, verbose=self.verbose)
    ...
    def detach (self):
        ...
        utils.dbgPrint("[*] Detaching from debuggee", Fore.GREEN, verbose=self.verbose)
    ...
    def event_handler_create_thread (self):
        ...
        utils.dbgPrint("[*] CREATE_THREAD_DEBUG_EVENT (%d)" % self.dbg.dwThreadId, Fore.GREEN, verbose=self.verbose)
    ...
    def exception_handler_breakpoint (self):
        ...
        utils.dbgPrint("[*] Breakpoint handler triggered at %08x from thread id %d" % (self.exception_address, self.dbg.dwThreadId), Fore.GREEN, verbose=self.verbose)
        ...
        if not self.bp_is_ours(self.exception_address):
            ...
            # ignore all other breakpoints we didn't explicitly set.
                ...
                utils.dbgPrint("[*] Breakpoint not ours %08x" % self.exception_address, Fore.GREEN, verbose=self.verbose)
                continue_status = DBG_EXCEPTION_NOT_HANDLED

        # breakpoints we did set.
        else:
            # restore the original byte at the breakpoint address.
            utils.dbgPrint("[*] Restoring original byte at %08x" % self.exception_address, Fore.GREEN, verbose=self.verbose)
    ...
    def exception_handler_guard_page (self):
        ...
        utils.dbgPrint("[*] EXCEPTION_GUARD_PAGE()", Fore.GREEN, verbose=self.verbose)
        ...
        if self.memory_breakpoint_hit:
            utils.dbgPrint("[*] Direct hit on memory breakpoint at %08x" % self.memory_breakpoint_hit, Fore.GREEN, verbose=self.verbose)

        if self.write_violation:
            utils.dbgPrint("[*] Write violation from %08x on %08x of mem bp" % (self.exception_address, self.violation_address), Fore.GREEN, verbose=self.verbose)
        else:
            utils.dbgPrint("[*] Read violation from %08x on %08x of mem bp" % (self.exception_address, self.violation_address), Fore.GREEN, verbose=self.verbose)
    ...
    def exception_handler_single_step (self):
        ...
        utils.dbgPrint("[*] EXCEPTION_SINGLE_STEP", Fore.YELLOW, verbose=self.verbose)
        ...
        if self._restore_breakpoint:
            ...
            if isinstance(bp, breakpoint):
                utils.dbgPrint("[*] Restoring breakpoint at 0x%08x" % bp.address, Fore.GREEN, verbose=self.verbose)
            ...
            # restore PAGE_GUARD for a memory breakpoint (make sure guards are not temporarily suspended).
                ...
                utils.dbgPrint("[*] Restoring %08x +PAGE_GUARD on page based @ %08x" % (bp.mbi.Protect, bp.mbi.BaseAddress), Fore.GREEN, verbose=self.verbose)
            ...
            # restore a hardware breakpoint.
                ...
                utils.dbgPrint("[*] Restoring hardware breakpoint on %08x" % bp.address, Fore.GREEN, verbose=self.verbose)
    ...
    def get_register (self, register):
        ...
        utils.dbgPrint("[*] Getting %s in thread id %d" % (register, self.dbg.dwThreadId), Fore.GREEN, verbose=self.verbose)
    ...
    def get_system_dll (self, idx):
        ...
        # utils.dbgPrint("[*] Getting system dll...", Fore.GREEN, verbose=self.verbose)
    ...
    def hide_debugger (self):
        ...
        utils.dbgPrint("[+] Debugger successfully hidden from debugee.", Fore.GREEN)
    ...
    def process_snapshot (self, mem_only=False):
        ...
        utils.dbgPrint("[*] Taking debuggee snapshot...", Fore.GREEN, verbose=self.verbose)
        ...
        if not mem_only:
            ...
            for thread_id in self.enumerate_threads():
                ...
                utils.dbgPrint("[*] Saving thread context of thread id: %08x" % thread_id, Fore.GREEN, verbose=self.verbose)
        ...
        while cursor < 0xFFFFFFFF:
            ...
            if save_block:
                utils.dbgPrint("[*] Adding %08x +%d to memory snapsnot." % (mbi.BaseAddress, mbi.RegionSize), Fore.GREEN, verbose=self.verbose)
    ...
    def resume_thread (self, thread_id):
        ...
        utils.dbgPrint("[*] Resuming thread: %08x" % thread_id, Fore.GREEN, verbose=self.verbose)
    ...
    def set_debugger_active (self, enable):
        ...
        utils.dbgPrint("[*] Setting debug event loop flag to %s" % enable, Fore.GREEN, verbose=self.verbose)
    ...
    def set_register (self, register, value):
        ...
        utils.dbgPrint("[*] Setting %s to %08x in thread id %d" % (register, value, self.dbg.dwThreadId), Fore.GREEN, verbose=self.verbose)
    ...
    def single_step (self, enable, thread_handle=None):
        ...
        utils.dbgPrint("[*] Single step %s" % enable, Fore.GREEN, verbose=self.verbose)
    ...
    def suspend_thread (self, thread_id):
        ...
        utils.dbgPrint("[*] Suspending thread: %08x" % thread_id, Fore.GREEN, verbose=self.verbose)
    ...
    def virtual_alloc (self, address, size, alloc_type, protection):
        ...
        if address:
            utils.dbgPrint("[*] VirtualAllocEx(%08x, %d, %08x, %08x)" % (address, size, alloc_type, protection), Fore.GREEN, verbose=self.verbose)
        else:
            utils.dbgPrint("[*] VirtualAllocEx(NULL, %d, %08x, %08x)" % (size, alloc_type, protection), Fore.GREEN, verbose=self.verbose)
    ...
    def virtual_free (self, address, size, free_type):
        ...
        utils.dbgPRint("[*] VirtualFreeEx(%08x, %d, %08x)" % (address, size, free_type), Fore.GREEN, verbose=self.verbose)
